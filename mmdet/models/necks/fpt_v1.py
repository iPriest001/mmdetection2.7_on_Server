    ...
    def __init__(self,
        ...
        if with_norm != 'none':
            ...
            self.fpt_p5 = nn.Sequential(
                *[nn.Conv2d(out_channels * 5, out_channels, 3, padding=1, bias=False), norm(out_channels)])  # *5 because 1(orignal) + 4 (results of FPT)  !!!# remove p2 --> *4
            ...
            self.fpt_p5 = nn.Conv2d(out_channels * 5, out_channels, 3, padding=1)  # *5 ecause 1+4  , !!!# remove p2 --> *4
            self.fpt_p4 = nn.Conv2d(out_channels * 5, out_channels, 3, padding=1)
            self.fpt_p3 = nn.Conv2d(out_channels * 5, out_channels, 3, padding=1)
            # add P6, P7
...
class MixtureOfSoftMax(nn.Module):
    ...
    def forward(self, qt, kt, vt):
        ...
        attn = self.softmax2(kt)  # factorize attention + MoS
...
class cross_scale_CoordAtt(nn.Module):
    def __init__(self, inp, oup, reduction=32):
        ...
        self.conv1_s2 = nn.Conv2d(inp, mip, kernel_size=3, stride=2, padding=1)
        self.conv1_s4 = nn.Sequential(nn.Conv2d(inp, mip, kernel_size=3, stride=2, padding=1, bias=False),
                                      nn.Conv2d(inp, mip, kernel_size=3, stride=2, padding=1, bias=False))
